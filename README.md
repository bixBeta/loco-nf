...
> itself are their work.
>
> Every Snakemake rule, R script and configuration default is byte-identical
> to upstream, so results are directly comparable. What this repository adds
> is packaging: a single Apptainer image holding every dependency, and a
> Nextflow front-end that builds loco-pipe’s input tables from a sample sheet.
>
> Please cite the original authors — see [Citation](#citation).

## Running this fork

Two things are added on top of loco-pipe, and they are independent: use the
image on its own, or drive it with Nextflow.

### The image

One Apptainer/Singularity image carries angsd, samtools, pcangsd, ohana,
Snakemake, and R 4.2 with tidyverse, cowplot, fitdistrplus, extraDistr,
data.table, RSpectra and lostruct. Nothing is built at run time.

``` bash
apptainer exec loco-pipe.sif loco-pipe-check
```

That resolves every binary the workflow calls and loads every R package the
scripts use. Build it, or check it after copying it to a new host, following
[containers/README.md](containers/README.md).

On the TREx server the image already sits at
`/workdir/TREx_shared/projects/loco-pipe.sif`, which is the default, so nothing
needs pointing at it. Anywhere else, pass `--sif /full/path/to/loco-pipe.sif`.

### The Nextflow front-end

loco-pipe reads two tab separated tables. The front-end writes them from one
comma separated sample sheet, validates everything at launch, and hands off:
...
`params.yaml` ships with every default, commented; anything on the command
line overrides it. A few things worth knowing:

- **Contigs come from the reference `.fai`**, not the sheet, filtered by
  `--minlen` (default 1 Mb). loco-pipe parallelizes by contig, so run
  `--listContigs` first to see what the reference holds and keep the count
  under about 100. A `--minlen` that excludes everything is an error, not a
...
sample sheet in full.

### One caveat

`loco-pipe start` calls `subprocess.run` without checking the return code, so
**it exits 0 even when the workflow failed**. The Nextflow front-end greps the
