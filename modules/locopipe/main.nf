...
END_VERSIONS
    ...
    if grep -qE 'WorkflowError|command exited with non-zero|MissingInputException|Error in rule|^Error' "\$log" ; then
        ...
        # Nextflow shows only the last stretch of a failed task's stderr, so
        # everything below is BOTH kept short and ordered with the most useful
        # part last: excerpt, then which rule, then what to do about it. The
        # whole thing is in .command.err and in the log either way.
        ...
        # end of the log the failure is hundreds of lines up and the last lines
        # are some other job's successful output. Show what snakemake wrote
        # about the failure itself, and fall back to the tail only if it wrote
        # nothing ( a failure before the DAG, say ).
        if grep -qE 'Error in rule|RuleException' "\$log" ; then
            grep -E -A 8 'Error in rule|RuleException' "\$log" | head -12 >&2
        else
            tail -12 "\$log" >&2
        fi

        # Which rule, before the guidance: the pointer is useless without the
        # advice, but the advice is actionable on its own.
