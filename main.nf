...
Args:
    ...
    * --lococonfig   : a COMPLETE loco-pipe config to use instead of the generated
                       one - upstream's config.yaml works as is:
                         https://github.com/sudmantlab/loco-pipe/blob/main/config.yaml
                       Its basedir, reference, scriptdir, sample_table, chr_table
                       and pop_level are rewritten to match this run, since
                       upstream ships them as /path/to/loco-pipe placeholders and
                       pop_level: species, which would not match the table this
                       front-end writes. Everything else is used as you wrote it.
                       Re-applied every run, so the file is the source of truth.
                       --locoparams still applies on top.
    * --dryrun       : lay the project out, apply the settings, and ask snakemake
                       what it WOULD run, without running any of it. The way to
                       check a real dataset is configured correctly before
                       committing to a long run. Implies no report.
...
config       : ${params.lococonfig ?: "generated by loco-pipe init"}
settings     : ${params.locoparams ?: "loco-pipe defaults"}
outdir       : ${file(params.outdir).toAbsolutePath().normalize()}
launch       : ${params.launch}${params.dryrun ? "   ( DRY RUN: nothing will be analysed )" : ""}
...
def checkInputs() {
    ...
    if( params.lococonfig ) {
        if( !file(params.lococonfig).exists() )
            error "No loco-pipe config at: ${params.lococonfig}"
        try {
            def y = new org.yaml.snakeyaml.Yaml().load(file(params.lococonfig).text)
            if( !(y instanceof Map) || !y.containsKey('global') )
                error """${params.lococonfig}: does not look like a loco-pipe config.
    It should be the whole file, with a 'global:' section - upstream's config.yaml
    works as is. For changing just a few settings, use --locoparams instead."""
        }
        catch( Exception e ) {
            error "${params.lococonfig}: could not be read as YAML - ${e.message}"
        }
    }
...
workflow RUN {
    ...
    def lococonfig = params.lococonfig ? file(params.lococonfig).text : ""
